...
なお、[Octkit](https://github.com/octokit)を使わずに実現すること。

**注意: GitHubのAPIトークンをソースコード上にハードコーディングしないこと**
...
また、クライアントからサイズの大きな `file.txt` をアップロードする際、`body`の設定に `fs.createReadStream` を利用した場合と `fs.read` を利用した場合で、クライアント側のメモリ使用量がどれだけ違うか確認しなさい。
